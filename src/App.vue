...
export default {
  ...
  setup() {
    ...
    const weather = ref({});
    const date = ref('');
    // 使用 computed 來定義一個響應式的計算屬性
    const currentDate = computed(() => {
      return dayjs().format('MMMM Do YYYY');
    });

    const fetchWeather = async () => {
      const response = await fetch(`${base_url.value}weather?q=${query.value}&units=metric&APPID=${api_key.value}`);
      weather.value = await response.json();  // 更新 weather ref 的值
      console.log(weather.value);  // 打印 weather 數據到控制台
    };
